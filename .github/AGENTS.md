...
- 구현 에이전트: `Electron 메인 개발자`, `프론트 개발자`, `코드 및 보안점검 문서 생성자`

## 아키텍처

- `main`: 시스템 권한이 필요한 기능 처리 (`src/main`)
- `preload`: 안전한 IPC 브릿지 제공 (`src/preload`)
- `renderer`: React UI 구현 (`src/renderer/src`)
...
- `.github/agents/backend-developer.agent.md` : `Electron 메인 개발자` (2단계)
...
- `.github/prompts/feature-plan-chain.prompt.md` : 6개 에이전트 순차 실행(리더→메인→프론트→품질→보안→문서)

## Skills

- `.github/skills/feature-plan-6step/SKILL.md` : 6단계 체인 실행 표준
