...
- 목적: 6개 에이전트 순차 실행(프로젝트 리더 → Electron 메인 개발자 → 프론트 개발자 → 코드 품질 리뷰어 → 보안 점검자 → 코드 및 보안점검 문서 생성자)
- 사용 예시:
  - `파일 내보내기 기능 추가, main: src/main/index.ts, preload: src/preload/index.ts, renderer: src/renderer/src/components/`
  - `기존 이미지 편집 패널 수정, IPC 채널 보안 강화, main/renderer 동시 변경`
...
- 신규 기능으로 대상 파일이 없으면 `Electron 메인 개발자`, `프론트 개발자` 단계에서 필요한 파일 생성 후 구현합니다.
- 신규 기능 검토 시 `코드 품질 리뷰어`, `보안 점검자`는 기존 코드 참조와 신규 대상 확정 이슈를 구분해 기록합니다.
